...
var $ptExtensions = GetIntrinsic('%Object.ptExtensions%');
...
module.exports = function SetIntegrityLevel(O, level) {
	...
	if (!$ptExtensions) {
		throw new $SyntaxError('SetIntegrityLevel requires native `Object.ptExtensions` support');
	}
	var status = $ptExtensions(O);
	if (!status) {
		return false;
	}
	if (!$gOPN) {
